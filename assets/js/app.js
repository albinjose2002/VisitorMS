...
function defaultState() {
  return {
    ...
    visitors: [
      {
        passId: "PASS-2026-00012",
        name: "Mariyam A M",
        type: "APPT",
        host: "Carol Solutions HR",
        purpose: "Final meeting",
        valid: "Today",
        status: "INSIDE",
        expired: false,
      },
      {
        passId: "PASS-2026-00015",
        name: "Rahul N",
        type: "WALKIN",
        host: "IT Dept",
        purpose: "Delivery",
...
function badgeLogAction(action) {
  if (action === "IN")
    return `<span class="badge-soft badge-good">Check-in</span>`;
  if (action === "OUT")
    return `<span class="badge-soft badge-warn">Check-out</span>`;
...
function badgeVisitorStatus(v) {
  ...
  if (v.status === "INSIDE")
    return `<span class="badge-soft badge-good">Inside</span>`;
  return `<span class="badge-soft">Outside</span>`;
}
function badgeApptStatus(s) {
  if (s === "ARRIVED")
    return `<span class="badge-soft badge-good">Arrived</span>`;
    ...
    <div class="glass h-100 p-2 d-flex flex-column">
      ...
      <div class="px-2 mb-2">
        ...
        <select id="gateSelect" class="form-select">
          <option value="${APP.gates.entry.id}" ${
    state.activeGate === APP.gates.entry.id ? "selected" : ""
  }>${APP.gates.entry.name}</option>
          <option value="${APP.gates.exit.id}"  ${
    state.activeGate === APP.gates.exit.id ? "selected" : ""
  }>${APP.gates.exit.name}</option>
        </select>
      </div>
  ...
  <div class="topbar">
    <div class="d-flex flex-wrap gap-2 align-items-center justify-content-between">
      <div class="d-flex align-items-center gap-2">
        <!-- toggle -->
        <button class="btn btn-outline-light d-lg-none rounded-3 align-items-center d-flex justify-content-center" style="width:40px; height:40px;" id="sidebarToggle">
        ...
        <div class="glass px-3 py-2 d-flex align-items-center gap-2">
          ...
          <span class="muted d-none d-md-inline">| Visitor Management</span>
        </div>
      </div>

      <div class="d-flex flex-wrap gap-2 align-items-center justify-content-end col-auto">
        <div class="pill">
        ...
        <a class="btn btn-accent rounded-4 px-3" href="walkin.html">
          ...
          <span class="d-none d-sm-inline">Issue Pass</span>
        </a>
  ...
  if (ctx1) {
    new Chart(ctx1, {
      ...
      data: {
        labels,
        datasets: [
          { label: "Check-ins", data: dataIn, tension: 0.35 },
          { label: "Check-outs", data: dataOut, tension: 0.35 },
        ],
      },
      options: {
...
function renderDashboard() {
  ...
  const inside = s.visitors.filter(
    (v) => v.status === "INSIDE" && !v.expired
  ).length;
  ...
  if (recent) {
    recent.innerHTML =
      ...
      <tr>
        ...
        <td>${
          l.gate === "GATE_ENTRY" ? APP.gates.entry.name : APP.gates.exit.name
        }</td>
...
function renderAppointments() {
  ...
  const q = (document.getElementById("apptSearch")?.value || "")
    .toLowerCase()
    .trim();
  const list = s.appointments.filter(
    (a) =>
...
function renderVisitors() {
  ...
  let list = [...s.visitors];
  if (mode === "INSIDE")
    list = list.filter((v) => v.status === "INSIDE" && !v.expired);
  if (mode === "APPT") list = list.filter((v) => v.type === "APPT");
  if (mode === "WALKIN") list = list.filter((v) => v.type === "WALKIN");
  if (q)
    list = list.filter((v) =>
      (v.passId + v.name + v.host + v.purpose).toLowerCase().includes(q)
    );
  ...
  tbody.innerHTML =
    ...
    <tr>
      ...
      <td>${
        v.type === "APPT"
          ? `<span class="badge-soft">Appointment</span>`
          : `<span class="badge-soft badge-warn">Walk-in</span>`
      }</td>
      <td class="muted">${v.host}</td>
      <td class="muted">${v.valid}</td>
      <td>${badgeVisitorStatus(v)}</td>
      <td class="text-end">
        <a class="btn btn-sm btn-outline-light rounded-4" href="scan.html?pass=${encodeURIComponent(
          v.passId
        )}">
          Scan
        </a>
...
function processScan() {
  ...
  if (!v) {
    show(
      "bad",
      "Pass not found",
      `No visitor pass exists for <b>${token}</b>.`
    );
  ...
  if (v.expired) {
    show(
      "bad",
      "Expired pass",
      `Pass <b>${v.passId}</b> for <b>${v.name}</b> is expired. Re-issue required.`
    );
  ...
  if (s.activeGate === APP.gates.entry.id) {
    if (v.status === "INSIDE") {
      show(
        "warn",
        "Already inside",
        `${v.name} is already inside. Duplicate entry blocked.`
      );
      ...
      show(
        "ok",
        "Check-in successful",
        `${v.name} checked in at <b>${gateLabel(s.activeGate)}</b>.`
      );
    ...
    if (v.status !== "INSIDE") {
      show(
        "warn",
        "Not inside",
        `${v.name} is not marked inside. Exit blocked.`
      );
      ...
      show(
        ...
        `${v.name} checked out at <b>${gateLabel(s.activeGate)}</b>.`
      );
...
function issuePassFromForm() {
  ...
  const name =
    (document.getElementById("pName")?.value || "").trim() || "Visitor";
  const host = (document.getElementById("pHost")?.value || "").trim() || "—";
  const purpose =
    (document.getElementById("pPurpose")?.value || "").trim() || "—";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (page === "visitors") {
    ...
    document
      ...
      ?.addEventListener("change", renderVisitors);
    document
      .getElementById("visitorSearch")
      ?.addEventListener("input", renderVisitors);
  }
  if (page === "logs") {
    renderLogs();
    document
      .getElementById("logGateFilter")
